...
exports.submitFeedback = function(reqBody, callback){
	...
	conn.executeSql(sql, [reqBody.user_id, reqBody.feedback, new Date(timestamp.toLocaleString().substr(0,17))], callback);
}
...
exports.updateUserBackgroundImage = function(user_id, url, callback){
	var sql = "update user_base_info set user_background_image_url = ? where user_id = ?";
...
exports.login = function(userPhone, password, callback) {
	var sql = 'select * from user_base_info where user_phone = ? and user_password = ?';
...
exports.updateLoginStatus = function(deviceToken, user_id){
	var sql = "update  user_base_info set device_token = ?, is_logout = ? where user_id = ?";
...
exports.updateUserFace = function(userId, facethumbnail, imageUrl, callback) {
	var sql = 'update user_base_info set user_facethumbnail = ?, user_face_image = ? where user_id = ?';
...
exports.insertUserImageInfo = function(userId, userImageURL, timeStamp, callback){
	var sql = 'insert into user_image_info (user_id, user_image_url, time_stamp) values (?,?,?)';
...
exports.getUserImage = function(userId, timestamp, count, callback){
	var sql = 'select * from image_url_v a where a.user_id = ? and a.timestamp<? order by timestamp desc limit ?';
...
exports.deleteUserImage = function(userId, userImageUrl, callback){
	var sql = 'delete from user_image_info where user_id= ? and user_image_url = ?';
...
exports.updateUserInfo = function(userInfo, callback){
	var sql = 'update user_base_info set User_career = ?, User_company = ?, User_sign = ?, User_interest = ? where User_id = ?';
	conn.executeSql(sql, [userInfo.user_career, userInfo.user_company, userInfo.user_sign, userInfo.user_interest, userInfo.user_id], callback);
}
...
exports.updateLocationInfo = function(locationInfo, callback){
	...
	var sql = 'update user_location_info set location_latitude = ?, location_longitude = ?, refresh_timestamp = ? where user_id = ?';
	conn.executeSql(sql, [locationInfo.latitude, locationInfo.longitude, locationInfo.timeStamp, locationInfo.user_id], callback);
}

exports.getNearbyPersonToken = function(user_id, callback){
	var sql = "select b.device_token from user_location_info a, user_base_info b where a.user_id<>? and a.user_id = b.user_id";
