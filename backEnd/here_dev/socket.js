...
function insertPrivateMsgAndPushToFront(msg, io) {
    ...
    instantMsgMgmt.insertPrivateMsg(msg.from, msg.to, msg.message, msg.msg_id, msg.msg_type, msg.datapath, msg.voice_time, msg.msg_srno, function (flag, result) {
        //check insert action
        if (!flag) {
            log.logPrint(config.logLevel.ERROR, result);
            ...
            instantMsgMgmt.getMsgByID(msg.msg_id, function (flag, result) {
                if (flag) {
                    log.logPrint(config.logLevel.DEBUG, 'getMsgByID SUCCESS');

                    redisClient.hget(userInfoHashKey, msg.to, function (err, reply) {
                        if (err) {
                            log.error(err, log.getFileNameAndLineNum(__filename));
                        } else {
                            if (reply != null) {
...
function startSocketServer() {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('register', function (msg, fn) {
            ...
            if (counter_id == null) {
                ...
                userMgmt.getMissedMsgRecord(user_id, function (flag, result) {
                    if (flag) {
                        if (result.length === 0) {
                            //insert first
                            userMgmt.insertMissedMsgRecord(user_id, function (flag, result) {
                                if (flag) {
                                    feedBack(flag, {}, fn);
                                } else {
                                    feedBack(flag, result, fn);
                ...
                instantMsgMgmt.getMissedMsg(user_id, counter_id, lastTimeStamp, function (flag, result) {
        ...
        socket.on('testACK', function (msg, callback) {
        ...
        socket.on('msg', function (msg, fn) {
            ...
            userMgmt.checkBlackList(msg.to, msg.from, function (flag, result) {
                if (flag) {
                    if (result.length > 0) {
                        ...
                        if(msg.msg_type == config.msgType.VOICEMSG
                            ||msg.msg_type == config.msgType.IMAGEMSG){
                            ...
                            fs.open(fileName, 'w', function (err, fd) {
                                if (err) {
                                    ...
                                    fs.write(fd, msg.data, 0, msg.data.length, function (err, written, buffer) {
                                        if (err) {
                                            log.error(err, log.getFileNameAndLineNum(__filename));
                                        } else {
        ...
        socket.on('disconnect', function (msg) {
            ...
            redisClient.hget(socketUserIdMap, socket.id, function (err, reply) {
                if (err) {
                    log.error(err, log.getFileNameAndLineNum(__filename));
                } else {
                    if (reply == null) {
