...
- (void)setContentModel:(ContentModel*)model
{
    ...
    for (int i=0; i<[model.imageModelArray count]; ++i) {
        ...
        [imageView loadingImageFromURL:imageModel.imageThumbnailStr];
...
- (void)contentImagePress:(UITapGestureRecognizer*)sender
{
    ...
    MyImageView* imageview = (MyImageView*)sender.view;
    originalImageView = imageview;
    
    if (imageview.loadingFlag == false) {
        [imageview reloading];
    }
    ...
    AppDelegate* app = (AppDelegate*)[[UIApplication sharedApplication] delegate];
    [app.tabBarViewController.view addSubview:backgroundView];
    
    NSURL* thumbnailURL = [originalImageView sd_imageURL];
    
    
    for (ImageModel* imageModel in myContentModel.imageModelArray) {
        if ([[thumbnailURL absoluteString] isEqual:imageModel.imageThumbnailStr]) {
            
            //[hud show]
            
            [enlargeImageview sd_setImageWithURL:[[NSURL alloc] initWithString:imageModel.imageUrlStr]  placeholderImage:enlargeImageview.image completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                //[hud hide:YES];
            }];
        }
    }
...
//    _parentViewController.tabBarController.tabBar.hidden = NO;
    ...
    [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
    [UIView animateWithDuration:0.3 animations:^{
        enlargeImageview.frame = [Tools relativeFrameForScreenWithView:originalImageView];
        [backgroundView setAlpha:0.0];
